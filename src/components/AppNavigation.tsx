...
const AppNavigation: React.FC = () => {
  ...
  const ProfileNavItem = ({ to, children }: { to: string; children: React.ReactNode }) => (
    <NavLink
      to={to}
      className={({ isActive }) =>
        `flex flex-col items-center justify-center transition-all relative ${
          ...
          <div className="relative">
            <img
              ...
              className={`w-6 h-6 rounded-full object-cover border transition-all ${
                isActive ? 'border-blue-600 dark:border-blue-400 scale-110' : 'border-gray-200 dark:border-gray-700'
              }`}
              alt="Profile"
            />
          </div>
    ...
    <>
      ...
      <div className="md:hidden fixed bottom-0 inset-x-0 bg-white dark:bg-black border-t border-gray-200 dark:border-gray-800 pb-safe">
        <div className="grid grid-cols-4 h-16">
          ...
          <ProfileNavItem to="/profile">Profile</ProfileNavItem>
        </div>
      </div>
    </>
  );
